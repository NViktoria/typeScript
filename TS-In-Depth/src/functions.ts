...
export function calcTotalPages(): void {
    ...
    const res = data.reduce((acc: bigint, obj) => {
...
export function createCustomer(name: string, age?: number, city?: string): void {
    ...
    if (age) {
        console.log(`Costumer age: ${age}`);
    }
    if (city) {
        console.log(`Costumer city: ${city}`);
    }
}

export function getBookByID(id1: Book['id']): BookOrUndefined {
    const books = getAllBooks();
    return books.find(({ id }) => id === id1);
}

export function сheckoutBooks(customer: string, ...bookIDs: number[]): string[] {
    console.log(`Costumer name: ${customer}`);
    return bookIDs
        .map(value => getBookByID(value))
        .filter(({ available }) => available)
        .map(({ title }) => title);
};

export function getTitles(autor: string): string[];
...
export function getTitles(...args: [string | boolean] | [number, boolean]): string[] {
    ...
    if (args.length === 1) {
        ...
        const [id1, available1] = args;
        if (typeof id1 === 'number' && typeof available1 === 'boolean') {
            return books.filter(({ id, available }) => id === id1 && available === available1).map(({ title }) => title);
        }
    }
...
export function getProperty(book: Book, prop: BookProperties): any | BookProperties {
